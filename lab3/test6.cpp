...
int main(){
    ...
    sin.sin_port = htons(10001);        // htons(): 2byte 데이터에 대해 호스트 byte 순서를 빅 엔디안 바이트 순서로 변환
    ...
    if (result == 0) {
        ...
        cout << "My port: " << ntohs(sin.sin_port) << endl;         // ntohs(): 2byte 데이터에 대해 호스트 byte 순서를 리틀 엔디안 바이트 순서로 변환
    }
